...
export function wireSession(managed: WirableManagedSession, session: Session, sink: EventSink, onAssistantTurn?: () => void): void {
  const id = managed.id;
  ...
  session.on("text", (ev) => {
    // Notify manager of top-level assistant turns (used for auto-naming)
    if (onAssistantTurn && !ev.parentToolUseId) {
      onAssistantTurn();
    }
